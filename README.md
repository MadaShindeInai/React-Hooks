...
4) useCallback
 ...
 6) useContext и useReducer
  ...
  ( переключились на useContext2)
  Переходим к редьюсеру. По хорошему, редьюсер - это функция, которая принимает стэйт и экшен, и возвращает новый стэйт. Важно: не изменяет существующий, а возвращает новый на основе старого!
  В зависимости от того, что мы делаем (увеличиваем, уменьшаем, обнуляем) стэйт, мы будем вызывать свой экшен, и c помощью payload изменять стэйт.
  Импортируем заранее заготовленные константы и с помощью конструкции switch case составляем наш редьюсер.
  Action - это объект в котором есть поле type - строка, и payload -  параметр, что мы передаем. Payload - это негласное соглашение названия данных, и нет ничего страшного, если вы назовете его подругому, и если полей 'payload' будет несколько.
  В случае count_up используем спрэд оператор и изменим только поле count.
  В случае count_down используем спрэд оператор и не будем использовать и передавать payload, а просто вычитать двойку.
  В случае count_zero в count засэтаем 0, в id новую дату, и не будем использовать и передавать payload.

  Отлично! Теперь попробуем заменить нашу текущую логику на Context API.
  Обернем компонент App в AppState, удалим оттуда State и не будем пробрасывать count в NeApp.
  Теперь переходим в компонент NeApp и получаем данные из стора(стэйт и диспатч) с помощью useContext который принимает наш контекст атрибутом,(предварительно импортировав наш контекст).
  Count сэтаем в див, а на onClick будем диспатчить action COUNT_UP.
  Проверяем все ли работает. 
  Теперь осталось добавить логику остальным компонентам.
  NeApp1 теперь будет отнимать двойку диспатча экшен COUNT_DOWN
  NeApp2 будет выводить сообщение что он ничего не делает. Я туда накинул логики для следующих примеров, она нас пока что не интересует. Важно то, что он не пробрасывает count в NeApp3.
  NeApp3 будет просто отправлять тип. Ему не нужен payload. 
  Проверяем - все работает, и пропсов стало гораздо меньше.
  Перейдем к кастомным хукам и рефакторингу. (Refactoring)

  7) Рефакторинг
  Первым делом - мне не нравится что мы пробрасываем функцию dispatch через useContext. Допустим в нашем приложении будет несколько контекстов, и тогда может возникнуть путаница, поэтому я предлагаю перенести объявление колбэков, что мы кладем в онКлики => в AppState. И сделать их доступными через Context положив в value.
  Мы могли бы отрефакторить функцию редьюсера, вынеся switch/case и заменив его объектом, но это может внести лишнюю путаницу и непонимание, поэтому оставим так.
  Лучше давайте займемся компонентом NeApp2. Там у нас появилась некая логика, которая выглядит в общем-то неплохо, но лежит вместе с рендером. 
  Хорошим тоном является разделение логики и рендера, поэтому давайте напишем свой хук и вынесем логику из компонента в него.
  Итак: у нас есть useEffect, который изменяет тайтл, есть useContext который в общем-то не используется в доме напрямую, и есть ultraString которая нужна нам в рендере. В этой же папке создаем файл, например useNeApp2.ts.
  Импортируем useEffect, useContext записываем базовый вид хука и просто переносим туда логику. В ретурне в объекте возвращаем ultraString , импортируем хук в NeApp2, вызываем его,и забираем оттуда ultraString. Логика и рендер - разделены. 
  NeApp сейчас не принимает пропсы. В случае, если что-то зависит от пропсов - мы можем принять это аргументом в наш хук, и использовать в нем.
  На этом все. Вопросы.