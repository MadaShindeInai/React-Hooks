...
В данный момент хук работает примерно как componentDidUpdate и componentDidMount вместе. 
Если мы хотим вызвать метод componentWillUnmount (например отменить подписку), мы должны вернуть коллбэк отвечающий за эти действия. Однако давайте посмотрим что прозойдет если мы не напишем return.(1)
